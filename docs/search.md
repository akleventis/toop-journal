...
toop-journal uses **SQLite FTS5** for full-text search over journal entries. The index is built and queried entirely in a **worker thread** so the main process event loop is never blocked during the build phase (~15s for 3,000 entries).
...
IPC: getEntriesForList() ← responds fast builds FTS5 virtual table...
IPC: getMostRecent()  ← responds fast    ...still building (~15s total)...
Renderer renders list                    posts { type:'ready', count:2919 }
...
`buildInMemoryFts()` in `main/db/sqlite.ts` creates the worker immediately after `createWindow()`. Only the DB path is sent via `workerData`:

```typescript
new Worker(workerPath, { workerData: { dbPath } });
```
...
The `fts5` virtual table tokenizes each insertion and builds the inverted index. This is the slow part (~15s for 3,000 entries). It runs entirely on Thread 2.
...
User submits query in List.tsx
  → db.searchEntries(query, limit?)     (renderer → IPC → main thread)
      → limit defaults to searchLimit from localStorage; undefined = all results
      → if a previous search is still pending, resolve it with [] (cancelled)
      → stores Promise { resolve, reject } in pendingSearch
      → worker.postMessage({ type:'search', query, limit: limit ?? -1 })
      → awaits Promise  [main event loop free to handle other IPC]

Worker receives message:
  → FTS5: SELECT id FROM entries_fts_mem WHERE entries_fts_mem MATCH ? ORDER BY timestamp DESC LIMIT ?
...
**Result limit** is configurable in More → Search Result Limit. Empty = all results. When a limit is set and results fill it exactly, a "Load more" button appears in the list footer — clicking it re-queries with `limit × page`.

### 5. Incremental sync

When an entry is created, updated, or deleted in the main thread, a fire-and-forget message keeps the worker's index in sync:
...
`closeDb()` calls `ftsWorker.terminate()`, which hard-kills the worker thread and releases its in-memory DB.
...
| Result limit | Configurable in More → Search Result Limit; empty = all results |
